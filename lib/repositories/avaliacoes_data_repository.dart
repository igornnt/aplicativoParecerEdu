...
class AvaliacoesDataRepository {
  ...
  Future<List<Avaliacao>> buscaTodas() async {
    ...
    return documentos
        .map((avaliacoes) => Avaliacao.fromFirestore(avaliacoes))
        .toList();
  }

  Future<List<Avaliacao>> buscaTodasPeloCriterio(
      DocumentReference criterioRef) async {
    var ref = this
        ._database
        .collection(this._caminho)
        .where('criterioRef', isEqualTo: criterioRef);
    List<DocumentSnapshot> documentos = new List<DocumentSnapshot>();
